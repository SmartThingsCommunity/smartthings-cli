...
describe('listing-io', () => {
	...
	describe('outputGenericListing', () => {
		it('calls outputItem when given idOrIndex', async () => {
			...
			await outputGenericListing<string, SimpleType, SimpleType>(command, 'id or index', listFunction, getFunction, translateToId, false)
			...
			expect(outputItemSpy).toHaveBeenCalledWith(command, expect.anything())
			...
			const getCaller: () => Promise<SimpleType> = outputItemSpy.mock.calls[0][1] as never
		...
		it('calls outputList when not given idOrIndex', async () => {
			...
			await outputGenericListing<string, SimpleType, SimpleType>(command, undefined, listFunction, getFunction, translateToId)
	...
	describe('outputListing', () => {
		...
		it('calls outputItem when given an id', async () => {
			...
			await outputListing(command, 'id or index', listFunction, getFunction)

			expect(translateToIdSpy).toHaveBeenCalledTimes(1)
			expect(translateToIdSpy).toHaveBeenCalledWith(command, 'id or index', listFunction)
			expect(outputItemSpy).toHaveBeenCalledTimes(1)
			expect(outputItemSpy).toHaveBeenCalledWith(command, expect.anything())
		...
		it('calls outputList when not given idOrIndex', async () => {
			...
			await outputListing(command, undefined, listFunction, getFunction, true)
