import { formatAndWriteList, type FormatAndWriteListOptions, type FormatAndWriteListConfig } from './format.js'
...
export type OutputListOptions = FormatAndWriteListOptions
export const outputList = async <L extends object>(
	command: SmartThingsCommand<OutputListFlags>,
	config: OutputListConfig<L>,
	getData: GetDataFunction<L[]>,
	options?: OutputListOptions,
): Promise<L[]> => {
	const list = config.sortKeyName ? sort(await getData(), config.sortKeyName) : await getData()
	await formatAndWriteList(command, config, list, options)
	return list
}
