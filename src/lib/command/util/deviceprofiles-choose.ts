import { type DeviceProfile, type LocaleReference, type SmartThingsClient } from '@smartthings/core-sdk'
...
export const chooseDeviceProfileFn = (
	...
	const listItems = async (client: SmartThingsClient): Promise<(DeviceProfile & WithLocales)[]> => {
		const deviceProfiles = await client.deviceProfiles.list()
		if (options?.verbose) {
			return await Promise.all(deviceProfiles.map(async deviceProfile => {
				const withLocales: DeviceProfile & WithLocales = { ...deviceProfile }
				try {
					const locales = await client.deviceProfiles.listLocales(deviceProfile.id)
