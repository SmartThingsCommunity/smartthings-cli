...
const builder = (yargs: Argv): Argv<CommandArgs> =>
	apiCommandBuilder(yargs)
		...
		// TODO: remove but implement later (already have JIRA issue)
		.option('hub', { describe: 'id of hub to connect to', type: 'string', conflicts: 'hub-address' })
		.option(
			'connect-timeout',
			{
...
const handler = async (argv: ArgumentsCamelCase<CommandArgs>): Promise<void> => {
	...
	const config: LiveLogClientConfig = {
		...
		verifier: (cert: PeerCertificate) => checkServerIdentity(command, authority, spinner, cert),
