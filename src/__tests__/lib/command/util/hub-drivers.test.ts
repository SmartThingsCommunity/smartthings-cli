...
describe('checkServerIdentity', () => {
	const spinnerStartMock = jest.fn<Ora['start']>()
	const spinnerStopMock = jest.fn<Ora['stop']>()
	const spinnerMock = { start: spinnerStartMock, stop: spinnerStopMock } as unknown as Ora
	...
	it('does nothing if hub already known', async ()  => {
		await checkServerIdentity(command, 'hub1', spinnerMock, certificate)
	...
	it('prompts user to confirm new host', async () => {
		...
		await checkServerIdentity(command, 'hub2', spinnerMock, hub2Certificate)

		expect(spinnerStopMock).toHaveBeenCalledExactlyOnceWith()
		...
		expect(spinnerStartMock).toHaveBeenCalledExactlyOnceWith()
	...
	it('fails if user rejects host', async () => {
		...
		expect(await checkServerIdentity(command, 'hub2', spinnerMock, hub2Certificate)).toBe('never return')
