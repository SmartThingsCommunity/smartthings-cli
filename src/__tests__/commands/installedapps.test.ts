...
import { withLocation, withLocations, WithLocation } from '../../lib/api-helpers.js'
...
describe('handler', () => {
	...
	it('lists details of a specified installed app', async () => {
		...
		expect(await getFunction('chosen-device-id')).toStrictEqual(installedApp)

		expect(apiInstalledAppsGetMock).toHaveBeenCalledExactlyOnceWith('chosen-device-id')
	...
	it('includes location for single app with verbose flag', async () => {
		...
		expect(await getFunction('chosen-device-id')).toBe(verboseInstalledApp)
